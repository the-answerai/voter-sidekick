...
const normalizeBill = async (billUrl, CongressApi) => {
  ...
  const latestTextVersionUrl = await CongressApi.getSourceContent(
    bill?.textVersions?.url,
    "text"
  );
  const latestPdfVersionUrl = await CongressApi.getSourceContent(
    bill?.textVersions?.url,
    "pdf"
  );
  const latestSummary = await CongressApi.getLatestSummary(
    bill?.summaries?.url,
    "pdf"
  );
  return {
    ...
    introducedDate: bill?.introducedDate
      ...
      : null,
    lastUpdatedDate: bill?.updateDate
      ? new Date(bill.updateDate).toISOString()
      : null,
...
async function getBillsFromCongress(congressNumber = null, billType = null) {
  try {
    ...
    const currentCongress =
      congressNumber || Math.floor((currentYear - 1789) / 2) + 1;
    ...
    const paginatedBills = await CongressApi.getBills({
      offset,
      limit,
    ...
    for (let i = 0; i < paginatedBills.length; i += batchSize) {
      ...
      const billsData = await Promise.all(
        batch.map(async (bill) => await normalizeBill(bill.url, CongressApi))
      );
