...
class CongressDownloader {
    ...
    async downloadBills(options = {}) {
        ...
        try {
            ...
            console.log(`Resuming from existing file. Current offset: ${existingData.offset}`);
        } catch (error) {
        ...
        while (true) {
            ...
            const response = await this.congressApi.makeRequest(url, { ...options, offset, limit });
            ...
            console.log(`Updated file: ${filePath} with ${response.bills.length} new bills. Total: ${existingData.bills.length} / ${response.pagination?.count}`);

            if (response.bills.length < limit || existingData.bills.length >= response.pagination?.count || options.testing) {
                break;
            }
        ...
        console.log(`Finished downloading bills for Congress ${congress}, bill type ${billType}`);
